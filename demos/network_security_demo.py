...
def demo_rate_limiting():
    ...
    for i in range(6):
        if limiter.is_allowed():
            ...
            print(f"{Colors.YELLOW}⏸️  Request {i+1}: Rate limited! "
...
def demo_network_security_manager():
    ...
    try:
        ...
        print(f"  {Colors.YELLOW}⏸️  Call 3: {demo_api_call('data3')}{Colors.RESET}")
...
def demo_secure_requests():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"{Colors.YELLOW}⚠️  Request returned status: {response.status_code}{Colors.RESET}")
    ...
    try:
        ...
        print(f"{Colors.YELLOW}⚠️  Unexpected success with self-signed cert{Colors.RESET}")
    except Exception as e:
